...
#'
#'
get_datasets <- function(datasets, error = F, named = T) {
  ...
  # Automatic return if only one dataset exists
  if (!hasArg(datasets) && length(Datasets) == 1 && datasets == names(Datasets)) {
    
    output <- names(Datasets)
    
    return(output)
    
  } else if (!hasArg(datasets)) {
    stop("Please specify a dataset.", call. = FALSE)
  }
  ...
  if (length(output) == 0) 
    warning("No dataset names matched.", .call = FALSE)
  
  if (named) 
    output <- cc(output)
  
  # Return vector of dtasets
  return(output)
...
#' @param observations vector declaring observation names (optionally named to 
#'  rename observations)
#' @param channel.mod name of channel modification (default = "SILAC")
#' @param data.frames names (prefixes or suffixes) by which to identify data 
#'  frames in the raw data sets
#' @param proteotypic.only only use proteotypic precursors
#' @param Q.Value precursor q-value threshold
...
#'
#' 
import2new_dataset_diann_precursor_SILAC <- function(
    raw.data = NULL, 
    variables.data = character(), 
    observations = character(), 
    channel.mod = "SILAC", 
